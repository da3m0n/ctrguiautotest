...
class Utils(object):
    ...
    def print_tree(cls, results_dir, test_type):
        ...
        # make  next_in_logs2 in for loop rather than walk()
        logs_dir = []
        # for file in dir_contents[1]:
        #     logs.append(dir_contents[0] + file + '/' + test_type + '/' + ipAddress)

        os.chdir(results_dir + '/logs/')

        for log_file in os.listdir(results_dir + '/logs/'):
            print('log_file', log_file)
            if os.path.isdir(log_file):
                logs_dir.append(log_file)
        ...
        for logs in logs_dir:
            ...
            field1.set("date", logs.replace('_', ' '))
            field1.set('sortDate', cls.reformat_date(logs))
            # for logs in logs_dir
            # print('results_dir: ' + results_dir + ' logs_dir: ' + logs_dir + ' test_type: ' + test_type)

            # print('dir', results_dir + '/logs/' + logs_dir + '/' + test_type + '/' + ipAddress)
            # next_in_logs = os.walk(results_dir + '/logs/' + logs + '/' + test_type + '/' + ipAddress).next()
            # print('next in logs', next_in_logs)

            print('test', results_dir + '/logs/' + logs + '/' + test_type + '/' + ipAddress)

            # next_in_logs = os.walk(results_dir + '/logs/' + logs_dir).next()
            # print('next_in_logs', next_in_logs)

            # for xmlfile in dir_contents[2]:
            # xmlfile = os.listdir(results_dir + '/logs/' + logs + '/' + test_type + '/' + ipAddress)[0]
            xmlfile = results_dir + '/logs/' + logs + '/' + test_type + '/' + ipAddress + '/' + logs + '.xml'
            # for xmlfile in next_in_logs[2]:
            print('xmlfile', xmlfile)
            field2 = ET.SubElement(field1, "fileName")
            field2.set("file", xmlfile.replace('_', ' '))
            field2.set('fileurl', '/logs/' + logs + '/' + test_type + '/' + ipAddress + '/' + xmlfile)

            # result = cls.extract_error_count(logs + '/' + test_type + '/' + ipAddress + '/' + xmlfile)
            # if len(next_in_logs[1]) > 0:
            #     in_date_files = os.walk(next_in_logs[0] + '/screenshots').next()
            #     print('in date files', in_date_files)
            ...
            #         field3.set('imageurl',
            #                    '/logs/' + logs + '/' + test_type + '/' + ipAddress + '/' + '/screenshots/' + image_name)
    ...
    def extract_error_count(cls, xmlfile):
        url = 'http://localhost:8000/logs/' + xmlfile
    ...
    def save_screenshot(self, test_name, test_type):
        ...
        screenshots_dir = self.pwd + '\\logs\\' + self.date + '\\' + test_type + '\\' + self.ipAddress + '\\screenshots'
